...
pub async fn change_password(user_id: Uuid, password: Secret<String>, pool: &PgPool) -> anyhow::Result<()> {
    ...
    .execute(pool)
    .await
    .context("Failed to perform query to update password in the database.")?;
    Ok(())
}
